...
layui.define(["jquery", "miniMenu", "element","miniTab", "miniTheme"], function (exports) {
    ...
    var miniAdmin = {
        ...
        render: function (options) {
            ...
            options.loadTabFlag = options.loadTabFlag || false;
            ...
            $.getJSON(options.iniUrl, function (data) {
                if (data == null) {
                    ...
                    // miniAdmin.renderHome(data.homeInfo); // 这里注释掉，在render方法中不执行初始化 首页的功能，所以在外面需要执行huangkun
                    miniAdmin.renderAnim(options.pageAnim); // 动画效果，注释掉就是去掉动画效果
                    miniAdmin.listen();
                    miniMenu.render({
        ...
        renderAnim: function (anim) {
            if (anim) {
                $('#layuimini-bg-color').after('<style id="layuimini-page-anim">' +
                    '.layui-tab-item.layui-show {animation:moveTop 0.5s;-webkit-animation:moveTop 0.5s;animation-fill-mode:both;-webkit-animation-fill-mode:both;position:relative;height:100%;-webkit-overflow-scrolling:touch;}\n' +
