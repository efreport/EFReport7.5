...
function initParams(xml, isSpreadRpt) {
    ...
    let serarch_show_btn_strhtml = '<div><input class="searchBtn" style="margin-left: 10px;" type="button" id="btn_setting_search" name="btn_setting_search" value="高级查询" /></div>';
    ...
    // paramObj.append("<div id='div_search_form'><form id='paramForm' action=''></form></div>");
        ...
        if (paramWidth == 0) {
            ...
            if (paramType == 1) { //文本编辑框
                ...
                if (defaultValue != "") { //存在默认值
                    ...
                    $.each(data, function (ii,ee) { //生成xmSelect渲染下拉多选所需的数据
                        let value = ee["ActualFieldData"].text(); //值
                        let text = ee["ShowFieldData"]; //文本
                        let dataObj = {
                            name: text,
                            value: value,
                ...
                $(this).find("Node:eq(0)").children().each(function (ii) {
                    ...
                    $(this).find("Node:eq(0)").children().each(function (iii) {
                        ...
                        let thrid_children = [];
                        $(this).find("Node:eq(0)").children().each(function (iiii) {
                            if (levelVal < 3) {
                                levelVal = 3;
                            }
                ...
                // console.log("数据层级--------levelVal-------->", levelVal);
                if (levelVal === 2) {

                    for (let k = 0; k < data.length; k++) {
                        for (let kj = 0; kj < data[k].sub.length; kj++) {
                            if (data[k].sub[kj].value === defaultValue) {
                                let _strval = data[k].name + " " + data[k].sub[kj].name;
                    ...
                    for (let k = 0; k < data.length; k++) {
                        for (let kj = 0; kj < data[k].sub.length; kj++) {
                            ...
                            for (let kjk = 0; kjk < data[k].sub[kj].sub.length; kjk++) {
                                if (data[k].sub[kj].sub[kjk].value === defaultValue) {
                                    let _strval = data[k].name + " " + data[k].sub[kj].name + " " + data[k].sub[kj].sub[kjk].name;
                                    $("#" + paramName).val(_strval);
                                }
                            }
                    ...
                    if (array_mypicker[selectedIndex[0]].hasOwnProperty('sub')) {
                        creatList(array_mypicker[selectedIndex[0]].sub, second);
                    } else {
                        second = [{text: '', value: 0}];
                    }

                    if (array_mypicker[selectedIndex[0]].sub[selectedIndex[1]].hasOwnProperty('sub')) {
                        creatList(array_mypicker[selectedIndex[0]].sub[selectedIndex[1]].sub, third);
                    } else {
                        third = [{text: '', value: 0}];
                    }
                    ...
                    picker.on('picker.change', function (index, selectedIndex) {
                        ...
                        function secondChange() {
                            ...
                            if (array_mypicker[first_index].sub[selectedIndex].hasOwnProperty('sub')) {
                                let secondCity = array_mypicker[first_index].sub[selectedIndex];
                                creatList(secondCity.sub, third);
                                picker.refillColumn(2, third);
                                picker.scrollColumn(2, 0)
                            } else {
                ...
                if (defaultValue != "") { //存在默认值
                        ...
                        $(this).find("Node:eq(0)").children().each(function (iii) {
                            ...
                            $(this).find("Node:eq(0)").children().each(function (iiii) {
                                ...
                                if ($.inArray($(this).find("ActualFieldData:eq(0)").text(), defaultArr) != -1) {
                                    ...
                                    default_show_val_ary.push($(this).find("ShowFieldData:eq(0)").text());
                    ...
                    if (default_show_val_ary.length > 0) {
                ...
                let tabAry = [
                    {
                        name: "北京市",
                        children: [{name: "朝阳区"}, {name: "东城区"}, {name: "西城区"}, {name: "崇文区"}, {name: "宣武区"}]
                    },
                    {
                        name: "湖北省",
                        children: [{name: "武汉市"}, {name: "黄石市"}, {name: "黄冈市"}, {name: "武穴市"}, {name: "鄂州市"},
                ...
                if (levelVal === 2) {
                    ...
                    // tabpickerHtml += '<div id="div_'+paramName+'" class="tabpicker" style="display: block;">';
                    tabpickerHtml += '<div id="div_' + paramName + '" class="tabpicker">';
    ...
    if (showColumnAry.length > 0) {
        ...
        $("#div_showcolumn input[name='btn_search']").click(function () {
            ...
            $("#div_showcolumn input:checkbox").each(function (idx, item) {
                if ($(this).attr('checked') || item.checked === true) {
                    strval += item.name + "=true;";
                } else {
                    strval += item.name + "=false;";
                }
                $("#div_showcolumn").hide();
            });
    ...
    $("#page_btn_search").click(function () {
        ...
        for (let i = 0; i < formJson.length; i++) {
            ...
            if (jObj.value !== null && jObj.value !== '') {
                if (jObj.name === 'hid_showcolumn') {
                    // console.log(jObj.name+"------------->",jObj.value);
                    // paramStr+=jObj.value;
                } else {
                    paramStr += jObj.name + "=" + jObj.value + ";";
                }
