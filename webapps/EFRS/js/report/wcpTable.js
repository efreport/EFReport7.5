...
function initWcpReport(tableId, sheetInfo) {
    ...
    $('#' + tableId).parent().css("overflow", "hidden"); //table的父级元素不显示滚动条
    if(kr == 1){ //如果保持横纵比
        vRatio = hRatio; //垂直方向放大比例等于水平方向放大比例
        $('#' + tableId).parent().css("overflow-y", "auto");
    }

    globalRatio[0] = hRatio;
    globalRatio[1] = vRatio;
    ...
    if(shapes != undefined){ //存在悬浮元素
        initShapes(shapes , hRatio, vRatio , tableId);
    }
...
function initWcpCell(td, cell, tableId , horizontalRatio , verticalRatio , sheetColorList , subSheetData) {
    ...
    if (cell.hasOwnProperty("HtmlFile")) { //子表单是否嵌套HTML文件
        td.html('<div style="width:' + tdWidth + 'px;height:' + tdHeight + 'px;box-sizing:border-box;"><span style="width: ' + tdWidth + 'px; height:  ' + tdHeight + 'px; display:flex;overflow:hidden;">' + '<iframe id="fra_' + cellProp[0]+ '_' + cellProp[1] +'" oh="' + tdHeight + '" ow="' + tdWidth + '" src="' + base + '/export' + '/' + cell.HtmlFile + '" style="width:100%;height:100%" frameborder=0 scrolling=no></iframe>' + '</span></div>');
    } else { //设置文本
...
function refreshWcpCell(td, cell, sheetName , horizontalRatio , verticalRatio , sheetColorList , subSheetData) {
...
//初始化悬浮插件子表单单元格
function initWcpFormSheetCell(td, cell, tableId , horizontalRatio , verticalRatio , sheetColorList , sheetFontList , data) {
    let cellProp = cell.N; //单元格属性
    //单元格不产生滚动条
    td.css("overflow-x", "hidden");
    td.css("overflow-y", "hidden");
