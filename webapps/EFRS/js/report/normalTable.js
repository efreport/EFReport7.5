...
function initNormalCell(td , cell , tableId , colorList){
   ...
   /* let tdWidth = td.attr('ow');  //单元格原始高度
    ...
    let fontSize; //字体大小与设计器大小有出入
    let hr = td.attr("hr");
    hr = parseFloat(hr);

    if(isAfs){ //联动时需要判断是否自适应字体
        fontSize = parseInt(cellProp[15] * hr);
    }else{
        fontSize = cellProp[15] + 3; //字体大小与设计器大小有出入
    }
    ...
    if (cellProp[3] * hr < 1) {
        ...
        leftBdWidth = Math.floor(cellProp[3] * hr);
    }
    //绘制左边框样式
    if (cellProp[2] != 0) {
    ...
    if (cellProp[6] * hr < 1) {
        ...
        topBdWidth = Math.floor(cellProp[6] * hr);
    }
    //绘制上边框样式
    if (cellProp[5] != 0) {
    ...
    if (cellProp[9] * hr < 1) {
        ...
        rightBdWidth = Math.floor(cellProp[9] * hr);
    }
    //绘制右边框样式
    if (cellProp[8] != 0) {
    ...
    if (cellProp[12] * hr < 1) {
        ...
        bottomBdWidth = Math.floor(cellProp[12] * hr);
    }
    //绘制下边框样式
    if (cellProp[11] != 0) {
    ...
    if (cell.hasOwnProperty("HyperLink")) { //超级链接属性
        ...
        td.html("<a href='javascript:;' onclick='Link.Fn.hyperlink(" + cell.HyperLink + ",event);'>" + htm + "</a>");
    }
    ...
    if (cell.hasOwnProperty("Pic")) {
        ...
        td.find('div').find('span').addClass('isImage');
    }
...
//初始化单元格
function refreshRegionCell(td , cell , tableId , colorList){
    let cellProp = cell.N; //单元格属性
    //填充单元格内容
    if (cell.hasOwnProperty("HtmlFile")) { //子表单是否嵌套HTML文件
        td.empty();
        ...
        td.find("div").find("span").text(text);
    }
}
...
function initNormalControllTd(tableId, td, cell) {
    ...
    if (!cell.hasOwnProperty("IsAllowEdit")) {//一般对应主键
        ...
        if (1 === props.ControlType) {
            ...
            widgetHtml = '<div class="layui-form"><select ctype="2"  lay-search="" id="' + tdId + '_sel"  title="' + hint + '" sel="2" vs="' + actualValue + '" reg="' + regions + '" name="' + showName + '">';
            //为单选下拉框添加option选项
        ...
        //按钮
            ...
            $.each(props.DataDict, function (_i, _n) {
                widgetHtml += '<input ctype="10"  name="' + name + '" reg="' + regions + '" type="radio" hint="' + hint + '" value="' + _n.Key + '" ' + (_n.Key == cell.V ? 'checked="checked"' : '') + '/>' + _n.Value + '<br>'; //ActualValue->V
            });
            widgetHtml += '';
            widgetHtml += '</div>';
            td.css({"text-align": ""});
            ...
            if (hasTime) { //日期时间
                laydate.render({
                    ...
                    type: 'datetime',
                        ...
                        let reg = $('#' + id).attr('reg');
                        if(reg != undefined){
                            Link.Fn.regionLink(this , 3 , reg , false ,'',value);
                        }
                        changeRowStatus(tdId);
                        let code = $('#' + id).data('changeCode');
                        if(code != undefined){
        ...
        $.each(multiSelectIdArray, function (k, v) {
            let treeObj = xmSelect.render({
                ...
                on: function (data) {
                    ...
                    let tdId = k.substr(0 , k.length - 5);//获取单元格ID
                    changeRowStatus(tdId);
                    let div = $('#' + k); //下拉多选渲染DIV
                    if(div.data('changeCode') != undefined){
                        executeFunction(div.data('changeCode'));
                    }
                },
            });
                ...
                $.each(events, function (index, element) {
                    ...
                    if (subcode != "") {
                        if (type == 6) {
                            ...
                            td.find('select[name="' + showName + '"]').data('changeCode', code);
                        }
                        if (type == 5) {
                            var fun;
                            try {
                ...
                $.each(events, function (index, element) {
                    ...
                    if (subcode != "") {
                        if (type == 8) {
                            ...
                            td.find('div[name="' + showName + '"]').data('changeCode', code);
                        }
                        if (type == 7) {
                            var fun;
                            try {
                                fun = new Function(code); //转化为函数
                                td.find('div[name="' + showName + '"]').bind('init', fun);
                                td.find('div[name="' + showName + '"]').trigger('init');
                                td.find('div[name="' + showName + '"]').attr('init', 'true');
                                td.find('div[name="' + showName + '"]').data('initCode', code);
                            } catch (err) {
                                console.log(err);
                                return true;
                            }
                ...
                $.each(events, function (index, element) {
                    ...
                    if (subcode != "") {
                        if (type == 15) {
                            ...
                            td.find('input[name="' + showName + '"]').data('changeCode', code);
                        }
                        if (type == 14) {
                            var fun;
                            try {
                ...
                $.each(events, function (index, element) {
                    ...
                    if (subcode != "") {
                        if (type == 13) {
                            ...
                            td.find('div').find('input[type="checkbox"]').data('changeCode', code);
                        }
                        if (type == 12) {
                            var fun;
                            try {
                                fun = new Function(code); //转化为函数
                                td.find('div').bind('init', fun);
                                td.find('div').trigger('init');
                                td.find('div').attr('init', 'true');
                                td.find('div').data('initCode', code);
                            } catch (err) {
                                console.log(err);
                                return true;
                            }
                ...
                $.each(events, function (index, element) {
                    ...
                    if (subcode != "") {
                        if (type == 17) {
                            ...
                            td.find('div').find('input[type="radio"]').data('changeCode', code);
                        }
                        if (type == 16) {
                            var fun;
                            try {
        ...
        form.on('select()', function(data){
            ...
            if(controlType != undefined){
                ...
                let id = $(data.elem).attr('id');
                //let tdId = id.substr(0 , id.length - 5);//获取单元格ID
                let tdId = id.substr(0 , id.length - 4);//获取单元格ID
                changeRowStatus(tdId);
                if(control.data('changeCode') != undefined){
                    let code = control.data('changeCode');
                    executeFunction(code);
                }
            }
        ...
        form.on('radio()', function(data){
            ...
            if(controlType != undefined){
                ...
                let tdId = id.substr(0 , name.length - 6);//获取单元格ID
                let changeCode = control.data('changeCode');
                if(changeCode != undefined){
                    executeFunction(changeCode);
                }
                changeRowStatus(tdId);
            }
        ...
        form.on('checkbox()', function(data){
            ...
            if(controlType != undefined){
                ...
                Link.Fn.regionLink(control , controlType , reg , false ,'');
                let name = $(data.elem).attr('name');
                let tdId = id.substr(0 , name.length - 9);//获取单元格ID
                let changeCode = control.data('changeCode');
                if(changeCode != undefined){
                    executeFunction(changeCode);
                }
...
function controlEvent(obj){
    ...
    if(obj.data('changeCode') != undefined){
        console.log(obj.data('changeCode'));
    }
}
    ...
    let uploadInst = upload.render({
        ...
        choose:function(obj){},
            ...
            if(res.state == 'success'){ //上传成功
                ...
                let fileNameHtml = '<div><input type="text" value="'+ id + "/" + fileName +'" style="display: none;"><img onclick="uploadImg(this)" style="width:'+ width +'px;height:'+ height +'px;" src="'+ base +'/uploadimages/'+ id + '/temp/' + fileName +'?id='+ randomId +'"></img></div>'
