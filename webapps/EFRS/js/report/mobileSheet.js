...
function initSheets(sheetInfos) {
    ...
    if (sheetInfos.length == 1) {//单SHEET报表
        ...
        $('#sheet').append(sheetHtml);//填充sheet工具栏区域
...
function refreshBySheet(index, sheetName) {
    ...
    if (sheetsPageInfos.length>0) {
        pageCurPageNo = sheetsPageInfos[index].curPageIdx;
    } else {
        pageCurPageNo = 1; // 默认第一页（huangkun）
    }
...
function generateSubSheet(td, cell , isForm) {
    ...
    if (sheetNames.length > 1) {//当关联多个子表单时，添加滚动DIV
        ...
        if(isShowSubReportScrollBar){
            carouselHtml =
                "<div id='"+ carouselId +"' class='layui-carousel' style='overflow:auto;width:" + parseInt(width) + "px;height:" + parseInt(height) + "px;margin:0px;padding:0px;'>" +
    ...
    $.each(cell.SubReportSheetNames, function (i, item) {
        ...
        $.getJSON(base + '/report/loadJSON?' + "t=" + new Date().getTime() + "&token=" + token, {
            ...
            //生成子表单Table
            generateSubSheetTable(td.attr("id"), data, itemId, i, item, isSubReportCellPercent, isSubReportKeepHVRatio , isShowSubReportScrollBar , scrollData , isShowSubReportScrollBar);
        });
    });
...
function generateSubSheetTable(tdId, data, itemId, index, sheetName, isWcp, isKr , isShowScroll , scrollData , isShowCenter) {
    ...
    var subTableId = tdId + "_" + index; //子报表对应的table ID
    ...
    let originHeight = $('#' + tdId).attr('oh'); //单元格原始高度
        ...
        //子表单table html
    let html = "<table id='" + subTableId + "' style='display:table;border: 0; border-collapse: collapse; border-spacing: 0;width:" + tdWidth + "px' index='" + index + "' sheetname='" + sheetName + "'></table>"; //子报表Table
    ...
    if($('#' + tdId).attr('hr') != undefined){
        ratios[0] = $('#' + tdId).attr('hr');
        ratios[1] =  $('#' + tdId).attr('vr');
    }else{
    ...
    $('#' + tdId).attr('hr' ,  ratios[0]);
    $('#' + tdId).attr('vr' , ratios[1]);
    //渲染table的每个单元格
    ...
    let divWidth = subTableObj.parent().width(); //外层div高度
    let tableTrueWidth = tableWidth + columns.Count + 1; //table真实宽度
    if(tableTrueWidth > divWidth){
        subTableObj.parent().css('width' , tableTrueWidth + 'px');
    }
    ...
    //解决区域联动后，table的宽度大于div，导致table边框无法显示的bug
   /* subTableObj.parent().css('width',tableWidth);
    subTableObj.parent().css('height',tableHeight)*/
    //绑定联动事件
    //Table.Fn.bindEvent(obj);
    //Height.Fn.adjust(false);
}