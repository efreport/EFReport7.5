...
function initWcpForm() {
    ...
    let hRatio = horizontalRatio(totalFWidth); //计算水平方向上的比例
    globalRatio[0] = hRatio;
    let vRatio = verticalRatio(totalFHeight); //计算垂直方向上的比例
    ...
    initWcpShapes(shapes, hRatio, vRatio);
    setContentHeight(); //设置内容区高度
}

//生成悬浮元素
function initWcpShapes(shapes, hRatio, vRatio) {
    //遍历悬浮元素
...
function generateWcpShapeSheet(tableId, data, element, width, height) {
    ...
    if (isSubReportKeepHVRatio) { //保持横纵比
        //显示垂直滚动条
        if(isShowScroll){
            table.parent().css('overflow-y', 'scroll');
        }else{
